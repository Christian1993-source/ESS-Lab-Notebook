...
const SUPABASE_TABLE = process.env.SUPABASE_TABLE || "ess_lab_reports";
...
const sectionOrder = [
  ...
  { type: "data", key: "variables", noteKey: "variablesNotes", label: "Variables" },
...
sectionOrder.forEach((section) => {
  if (!section.program) section.program = "ess";
});

const programSections = {
  ess: sectionOrder.filter((section) => section.program === "ess").map((section) => section.key)
};
...
function sanitizeReport(rawReport) {
  ...
  const sections = {};
  const program = programSections[report.program] ? report.program : "ess";
  ...
  return {
    ...
    program,
    activeSections: {
      ess: Array.isArray(report.activeSections?.ess)
        ? programSections.ess.filter((key) => report.activeSections.ess.includes(key))
        : programSections.ess
    },
    ...
    sections,
    tables: {
      variables: normalizeTableList(report.tables?.variables),
