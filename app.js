...
const REPORT_TOKEN_KEY = "essLabNotebook.reportToken";
...
const sectionKeys = [
  ...
  "methodology",
  "variablesNotes",
  "safetyEthicsEnvironment",
  "rawDataNotes",
  "processedDataNotes",
  "processedDataSampleCalculations",
  "presentingProcessedData",
  "analysisConclusion",
  "evaluation",
...
const sectionOrder = [
  ...
  { type: "text", key: "alternateHypothesis", label: "H1 Alternate Hypothesis" },
  { type: "text", key: "nullHypothesis", label: "H0 Null Hypothesis" },
  { type: "text", key: "strategy", label: "Environmental Strategy" },
  { type: "text", key: "methodology", label: "Methodology" },
  { type: "data", key: "variables", notesKey: "variablesNotes", label: "Variables" },
  { type: "text", key: "safetyEthicsEnvironment", label: "Safety, Ethical and Environmental Issues" },
  ...
  { type: "text", key: "bibliography", label: "Bibliography" }
];
...
const PROGRAM_CONFIGS = {
  ess: {
    ...
    sections: [
      ...
      "presentingProcessedData", "analysisConclusion", "evaluation", "weaknessesLimitations",
...
const scienceTableTemplates = {
  variables: ["Variable Type", "Variable", "Units", "Range / Effect and Control"],
  rawData: ["Trial", "", "", "", ""],
  processedData: ["Trial", "", "", "", ""]
};
...
const state = {
  ...
  tables: {
    variables: defaultTableList("variables"),
...
const elements = {
  ...
  variablesEditor: document.getElementById("variablesEditor"),
...
const sectionInputs = {
  ...
  alternateHypothesis: document.getElementById("section-alternateHypothesis"),
  nullHypothesis: document.getElementById("section-nullHypothesis"),
  strategy: document.getElementById("section-strategy"),
  methodology: document.getElementById("section-methodology"),
  variablesNotes: document.getElementById("section-variablesNotes"),
  safetyEthicsEnvironment: document.getElementById("section-safetyEthicsEnvironment"),
  ...
  presentingProcessedData: document.getElementById("section-presentingProcessedData"),
  analysisConclusion: document.getElementById("section-analysisConclusion"),
  evaluation: document.getElementById("section-evaluation"),
  weaknessesLimitations: document.getElementById("section-weaknessesLimitations"),
  improvements: document.getElementById("section-improvements"),
  bibliography: document.getElementById("section-bibliography")
};
...
function isTrialTable(tableKey) {
  return tableKey === "rawData" || tableKey === "processedData";
}
...
function getDefaultHeaders(tableKey, width) {
  ...
  if (tableKey === "variables") {
    return Array.from({ length: safeWidth }, (_, index) => template[index] ?? `Column ${index + 1}`);
  }
...
function buildInitialRows(tableKey, columnCount) {
  if (tableKey === "variables") {
    return ["Independent Variable", "Dependent Variable", "Control Variable 1", "Control Variable 2", "Control Variable 3"]
      .map((label) => buildEmptyRow(columnCount, label));
  }
  if (isTrialTable(tableKey)) {
...
function renderProgramUI() {
  ...
  const active = state.activeSections[state.program];
  document.body.classList.toggle("program-ess", state.program === "ess");
  document.querySelectorAll(".program-option").forEach((button) => {
...
function loadExampleReport() {
  ...
  const exampleReport = getEssExampleReport();
...
function buildPrintableSections(report) {
  const sections = [];
  const program = PROGRAM_CONFIGS[report.program] ? report.program : "ess";
...
function renderTableEditor(tableKey, container) {
  ...
  tableList.forEach((tableData, tableIndex) => {
    ...
    addRowBtn.addEventListener("click", () => {
      const trialLabel = isTrialTable(tableKey) && isTrialHeader(tableData.headers[0]) ? `Trial ${getNextTrialNumber(tableData.rows)}` : "";
      tableData.rows.push(buildEmptyRow(tableData.headers.length, trialLabel));
      onTableChange();
      renderTableEditor(tableKey, container);
    });
    ...
    deleteColumnBtn.addEventListener("click", () => {
      if (tableData.headers.length <= 1) {
        tableData.headers = isTrialTable(tableKey) ? ["Trial"] : ["Column 1"];
        tableData.rows = tableData.rows.map(() => buildEmptyRow(1, isTrialTable(tableKey) ? "Trial 1" : ""));
      } else {
      ...
      removeColumnBtn.addEventListener("click", () => {
        if (tableData.headers.length <= 1) {
          ...
          tableData.rows = tableData.rows.map((row) => {
            ...
            return buildEmptyRow(1, isTrialTable(tableKey) ? "Trial 1" : "");
          });
        }
        onTableChange();
        renderTableEditor(tableKey, container);
    ...
    tableData.rows.forEach((row, rowIndex) => {
      ...
      removeRowBtn.addEventListener("click", () => {
        if (tableData.rows.length === 1) {
          const trialLabel = isTrialTable(tableKey) && isTrialHeader(tableData.headers[0]) ? "Trial 1" : "";
...
function collectReport() {
  ...
  return {
    ...
    sections,
    tables: {
      variables: state.tables.variables,
...
function applyReportToUI(report) {
  ...
  state.tables.variables = normalizeTableList(normalizedReport.tables?.variables, "variables");
